# Torch Monkey 使用文档 / Usage Guide

> Wota-艺（ヲタ芸）3D 编排与可视化软件 · Based on 3D reconstruction & rendering.
> 版本 0.1 · 2026-06-25
...
- 实时预览带**荧光棒光轨（サイリウム）+ Bloom 暗环境**的演出效果；
- 导出编排结果为 `.tmonkey` 工程文件 / glTF / BVH。

四层解耦（参考 MMD）：**模型 / 动作 / 相机 / 特效** 完全独立，可自由混搭。
...
- 卡点（キメ）瞬间会自动触发 **Bloom 增强 punch** + 拖尾快速收敛（演出感的核心）。
